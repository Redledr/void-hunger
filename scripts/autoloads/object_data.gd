...
#
# Centralises all spawnable-object definitions and related helpers.
extends Node
...
# Cached sorted key list — built once, never mutated.
...
# Returns a pre-sorted copy (mass ascending) of all type keys.
# Use the cached array directly when read-only access is acceptable.
func get_sorted_types() -> Array:
	return _sorted_types

# Returns the 0-based index of a type in the mass-sorted list, or -1.
